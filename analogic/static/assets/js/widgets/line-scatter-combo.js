...
<div class="ks-chart-holder" style="${this.getGeneralStyles(d).join('')}">
    ...
    initEventHandlers() {
        ...
        c.parent().next().on('click', '.ChartLegendTag', e => {
            ...
            const id = e.data('id'), isHidden = !chart.isDatasetVisible(id), d = this.value.datasets[id], c = d.color;
            ...
            chart.getDatasetMeta(id).hidden = !isHidden;

            if (d.staticDatasetId && d.showLineForStaticPoints) {
                chart.data.datasets[d.staticDatasetId].showLine = !isHidden;
            }
    ...
    getChartConfig() {
        const data = [], xEdges = [], v = this.value, selectedDatasetIds = [], datasets = v.datasets, verticalLineXCoords = [], id = this.id, len = datasets.length;
        ...
        let e, f, yMin = Infinity, yMax = -Infinity, c, d, r, o, i, j = 0, hasSelectedDataset = selectedDatasetIds.length > 0;

        datasets.forEach((e, i) => e.selected && selectedDatasetIds.push(i));
        ...
        for (i = 0; i < datasets.length; ++i) {
            ...
            if (hasSelectedDataset && !e.static) {
                if (selectedDatasetIds.includes(i)) {
                    verticalLineXCoords.push(...d.map(e => e.x));
                } else {
                    e.static = true;
        ...
        for (e of Utils.filterUnique(verticalLineXCoords)) {
            data.push({
                data: [{x: e, y: yMin}, {x: e, y: yMax}],
                fill: false,
                showLine: true,
                borderColor: 'rgba(162,228,184,0.2)',
                pointRadius: 0,
                pointHitRadius: 0,
                pointHoverRadius: 0
            });
