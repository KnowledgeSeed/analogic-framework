/* global app, Auth, Doc, El, Pivot, QB, Sortable, Utils, Widget */

'use strict';
class PivotTableWidget extends Widget {

    getHtml(widgets, data) {
        const o = this.options, p = data[1] ? JSON.parse(data[1]) : null, s = (p || [])[4] || {};
...
<div id="pivot"><div class="ks-pivot-table-controls-holder">
    <div class="ks-pivot-table-control-row presets-row">
        <div class="ks-pivot-table-presets-btn">
            ...
            <div id="pivotPresetsDropdown" class="ks-pivot-table-presets-dropdown">
                ...
                <a class="supressZeroBtn" data-id="${o.id}" data-type="nonEmptyColumns" data-action="savePivotPreset"><span class="icon-check ${s.nonEmptyColumns ? '' : 'off'}" style="color: #1d7bff;"></span>From columns</a>
                <a class="supressZeroBtn" data-id="${o.id}" data-type="nonEmptyRows" data-action="savePivotPreset"><span class="icon-check ${s.nonEmptyRows ? '' : 'off'}" style="color: #1d7bff;"></span>From rows</a>
    ...
    initEventHandlers() {
        ...
        section = section.children().off('click').on('click', () => this.closeDropdowns());
    ...
    initPresetsDropdown() {
        ...
        this.pivotPresetsDropdown.parent().off('click')
            ...
            .on('click', '#loadPresetBtn', () => this.loadPreset())
    ...
    renderNextSelectorTreeLevel(nextLevelData, cols, callback) {
        ...
        if (isCheckableCol) {
            ...
            for (name in children) {
                ...
                h += '<div class="ks-pivot-tag-add-item" data-name="' + Utils.htmlEncode(name) + '">' + check + (aliasAttrName ? (name + ' (' + el[aliasAttrName] + ')') : name) + del + chevron + '</div>';
            }
        } else {
            for (name in children) {
    ...
    saveNewCard() {
        ...
        this.taintedCard = this.taintedHolder.children(cardSelector + (isTaintedHolderSlicer ? '[data-element="' + element + '"]' : ''));
    ...
    deleteCard(e) {
        ...
        this.popup = $('<div class="ks-pivot-tag-add-popup"><h3>Do you really want to delete card?</h3><div class="ks-pivot-tag-add-popup-button-holder"><a class="ks-pivot-btn btn-blue-light">No</a><a data-action="yes" class="ks-pivot-btn btn-blue">Yes</a></div></div>');
    ...
    saveAsNewSubset() {
        ...
        this.popup = $('<div class="ks-pivot-tag-add-popup"><h3>Save as New Subset</h3><label>Subset Title</label><input type="text"><div class="ks-pivot-tag-add-popup-button-holder"><a class="ks-pivot-btn btn-blue-light">Cancel</a><a data-action="save" class="ks-pivot-btn btn-blue">Save</a></div></div>');
            ...
            if ('save' === e.data('action')) {
                ...
                if (newSubsetName) {
                    ...
                    if (n) {
                        ...
                        this.popup.children('label').html('There is an existing privete subset with the name "' + n + '".<br>Would you like to owerwrite it with the new selection?');
    ...
    doSaveAsNewSubset(newSubsetName) {
        ...
        $.when(Pivot.call({data: d})).then(subsetData => this.reloadSubsetsInSelectorTree(cols, d, subsetData));
    }

    removeSubset(subset) {
        this.popup = $('<div class="ks-pivot-tag-add-popup"><div style="text-align: center;margin: 15px;"><span class="icon-trash-fill" style="color: #DC3545;font-size: 30px;"></span></div><h2 style="text-align: center;">Delete Subset</h2><h3>Are you sure?<br>This action cannot be undone!</h3><div class="ks-pivot-tag-add-popup-button-holder"><a class="ks-pivot-btn btn-blue-light">Cancel</a><a data-action="yes" class="ks-pivot-btn btn-red-bg">Delete</a></div></div>');
    ...
    doRemoveSubset(subset) {
        ...
        $.when(Pivot.call({data: d})).then(subsetData => {
    ...
    supressZeroBtnClicked(e) {
        let btn = $(e.currentTarget), s = btn.children('span'), isSupressed = !s.hasClass('off');

        s.toggleClass('off', isSupressed);
    ...
    cardClicked(e) {
        ...
        if (card.parent('#pivotSlicer').length) {
            card.children('#slicerFilterBox').length ? this.closeSlicerFilterBox() : this.createSlicerFilterBox(card);
        } else {
    ...
    doCreateSlicerFilterBox(card, elements) {
        ...
        e = $('<div id="slicerFilterBox" class="ks-pivot-table-presets-dropdown"><div class="ks-pivot-add-tag-search"><span class="icon-search"></span><input type="text" placeholder="Search..."></div><div class="ks-pivot-scrollable">' + h + '</div><hr><a class="ks-pivot-btn btn-blue-light">Edit List</a><a class="ks-pivot-btn btn-red">Delete Filter</a></div>');
        ...
        card.addClass('no-drag').append(e).promise().then(() => n.scrollTop(elements.eq(j).position().top - 100));
