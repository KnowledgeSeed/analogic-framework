...
class TestCamEmptyRepoAuthenticationProvider(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.auth_provider = cls.app.analogic_applications.get('camemptyrepo')

    def test_logged_in(self):
        with self.client:
            response = self.client.get('/camemptyrepo/logout')
            assert response.status_code == 200
            response = self.client.get("/camemptyrepo", follow_redirects=True)
            assert response.status_code == 200
            assert response.request.path == '/camemptyrepo/'
            assert response.request.base_url == 'http://localhost/camemptyrepo/'
    ...
    def test_auth(self):
        with self.client:
            with patch.object(analogic.cam.Cam, 'set_tm1_service', return_value='dddd') as mock_method:
                response = self.client.post('/camemptyrepo/auth', data={'c_pp': 'fsdlfj'})
    ...
    def test_auth_follow(self):
        with self.client:
            with patch.object(analogic.cam.Cam, 'set_tm1_service', return_value='dddd') as mock_method1, patch.object(
                ...
                response = self.client.post('/camemptyrepo/auth', data={'c_pp': 'fsdlfj'}, follow_redirects=True)
                assert response.status_code == 200
                assert 'redirect.js' not in response.text
    ...
    def test_call_client_side_mdx(self):
        with self.client:
            ...
            url = '/camemptyrepo/proxy/api/v1/ExecuteMDX?$expand=Axes($expand=Tuples($expand=Members($select=Name,%20Attributes))),Cells($select=Ordinal,Value)'
    ...
    def test_logout(self):
        with self.client:
            ...
            response = self.client.get("/camemptyrepo/pivot")
            self.assertTrue(response.status_code == 401)

            response = self.client.get("/camemptyrepo/export")
            self.assertTrue(response.status_code == 401)

            response = self.client.get("/camemptyrepo/activeUser")
            self.assertTrue(response.status_code == 401)

            response = self.client.get("/camemptyrepo/proxy/test")
            self.assertTrue(response.status_code == 401)
    ...
    def login_into_app(self):
        ...
        response = self.client.post('/camemptyrepo/auth', data={'c_pp': cam_passport}, follow_redirects=True)
        self.assertTrue(response.status_code == 200)
