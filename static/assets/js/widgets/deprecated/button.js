...
class OldButtonWidget extends Widget {
        ...
        if (o.hasLayout === true) {
            ...
            return `<div class="row margin-bottom-row">
                    <div class="col-2">
                        <div class="row">
                            <div class="col-${w}">
                                <a ${o.confirmMessage ? `data-confirmmessage="${o.confirmMessage}" ` : ''} ${o.url ? `target="_blank" href="${o.url}"` : `data-id="${o.id}" data-action="${o.action}"`}  class="${o.backgroundVisible ? `widget-btn button-widget${o.url ? '-url' : ''}` : `widget-link-cancel button-widget-cancel${o.url ? '-url' : ''}`}">${o.label}</a>
                            </div>
                        </div>
                    </div>
                </div>`;
    ...
    initEventHandlers(section) {
        if (section.find('a').data('confirmmessage')) {
            ...
            Widget.handleSystemEvent(section, 'click', '.button-widget-cancel');
        }
    }
}
;