...
class OldHorizontalTableWidget extends Widget {
    ...
    getHtml(widgets, data, withState, leftRowWidgets, rightRowWidgets) {
        ...
        if (widgets.length > 0) {
            s.push(`<div class="row margin-bottom-row">
                <div class="col">
            
                      <div class="row">${widgets.join('')} </div>
                </div>
            </div>`);
        }

        if (o.tooltip) {
            s.push(`<div class="row" style="margin-bottom: 30px;">
                        <div class="col">
                            <div style="width: 10px;" data-title="${o.tooltipTitle}" data-tooltip="${o.tooltip}" class="tooltip-icon"><span class="icon-info"></span></div>
			</div>
                    </div>`);
        }
    ...
    initEventHandlers(section, withState) {
        ...
        section.find('.sortable').each(function () {
            let table = $(this).closest('section').find('table');
            ...
            th.click(function () {
                table.find('th').removeClass('ordered-by').removeClass('asc').removeClass('desc');
                table.find('th').find('span').remove();
        ...
        section.find('td.editable').on('click', e => {
            OldHorizontalTableWidget.addEdit(e);
        });
