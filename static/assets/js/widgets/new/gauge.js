...
class GaugeWidget extends Widget {
    ...
    getHtml(widgets, d) {
        ...
        if (!d) {
            d = {values: [9.0, 9.1, 10], labels: ['Modelled', 'Plan', 'Target'], minRange: 0, maxRange: 14, title: 2020};
        }
...
<div class="ks-gauge" style="${this.getGeneralStyles(d).join('')}">
    ...
    initEventHandlers(section) {
        const v = this.value, canvas = $('#' + v.canvasId), ctx = canvas[0].getContext('2d'), c = new Chart(ctx, this.getGaugeConfig());

        this.initShowInFullScreenEventHandler(section, v);
    }

    initShowInFullScreenEventHandler(section, v) {
        $('#' + v.id + 'FullScreenBtn').on('click', () => {
            app.fn.showInFullScreen(v.title, '').promise().done(() => {
    ...
    getGaugeConfig() {
        const d = this.value, min = d.minRange, max = d.maxRange, values = d.values, datasets = [], colors = this.options.colors, len = max.toString().length;
...
Chart.controllers.gauge = Chart.controllers.doughnut.extend({
    ...
    draw: function () {
        ...
        const min = chartData.minRange, max = chartData.maxRange, step = (max - min) / 5 === 0 ? 1 : (max - min) / 5, labels = [];
