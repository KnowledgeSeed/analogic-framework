...
class RadioButtonRowWidget extends Widget {
    ...
    getHtml(widgets, data, withState) {
        ...
        if (data.active === true) {
            s.push(`<div class="ks-horizontal-table-cell-inner"><div data-index="${data.index}" class="ks-horizontal-table-row-toggle ${data.on ? 'ks-on' : '' }" ${data.d.join('')} id="${o.id + (data.num + 1)}" data-id="${data.id}" data-action="${o.action}"><div class="ks-horizontal-table-toggle-icon ks-horizontal-table-toggle-icon-off"><span class="icon-radio-off"></span></div><div class="ks-horizontal-table-toggle-icon ks-horizontal-table-toggle-icon-on"><span class="icon-radio-on"></span></div></div></div>`);
        }
