...
class ToggleWidget extends Widget {
    ...
    getHtml(widgets, d) {
        ...
        let mainDivClass = [], mainDivStyle = this.getGeneralStyles(d), titleStyles = [];

        v.titleOn !== '' && mainDivClass.push('has-label');
        '1' == v.value && mainDivClass.push('ks-on');
        ...
        if (v.groupId && '1' == v.value) {
            app.widgetValue[v.groupId] = {ordinal: d.ordinal, value: v.titleOn};
        }
        ...
        return `
                <div class="ks-toggle ${mainDivClass.join(' ')} ${v.groupId ? `ks-toggle-${v.groupId}` : ''} ${v.isGridTableHierarchyExpander ? 'ks-toggle-expander' : ''}" style="${mainDivStyle.join('')}" data-ordinal="${d.ordinal}" data-value="${v.value}" data-id="${o.id}" data-action="launch">
                    <div class="ks-toggle-inner ${v.editable === false ? 'readonly' : ''}">
                            ...
                            <div style="${titleStyles.join('')}" class="ks-toggle-label ks-toggle-label-on">${v.titleOn}</div>
                            <div style="${titleStyles.join('')}" class="ks-toggle-label ks-toggle-label-off">${v.titleOff}</div>
                    </div>
                </div>`;
    }
        ...
        section.find('.ks-toggle').on('click', e => {
            ...
            if (o.groupId) {
                if (isActive) {
                    ...
                    app.widgetValue[o.groupId] = {ordinal: s.data('ordinal'), value: s.find('.ks-toggle-label-on').html()};

                    app.fn.handleSystemEvent('launch.' + o.groupId, {}, {});
                }
            } else {
                s.toggleClass('ks-on', isActive).trigger('change', [isActive]);
    ...
    static doExpand(s, isActive, toggleIndex, fade = true) {
        ...
        console.log(s, currentPadding, toggleIndex);
